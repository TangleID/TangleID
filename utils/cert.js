...
const generateAddress = async (seed) => {
  const p = new Promise((res, rej) => {
    iota.api.getNewAddress(seed, { index: 0 }, (e, s) => { // eslint-disable-line consistent-return
      if (e) return rej(e);
      res(s);
    });
  });
...
const sendTransfer = async (seed, transfers) => {
  const p = new Promise((res, rej) => {
    ...
    iota.api.sendTransfer(seed, 6, 15, transfers, (e, s) => { // eslint-disable-line
      if (e) return rej(e);
      res(s);
    });
  });
...
const find = async (query) => {
  const p = new Promise((res, rej) => {
    iota.api.findTransactions(query, (e, s) => { // eslint-disable-line consistent-return
      if (e) return rej(e);
      res(s);
    });
  });
...
const getTransactions = async (hashes) => {
  const p = new Promise((res, rej) => {
    iota.api.getTransactionsObjects(hashes, (e, s) => { // eslint-disable-line consistent-return
      if (e) return rej(e);
      res(s);
    });
  });
